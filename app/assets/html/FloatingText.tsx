...
export default function FloatingText({ rows: rowsProp = 28, cols: colsProp = 50, className }: FloatingTextProps) {
  const propBoxRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    const TARGET_ROW_W = (440 / 50) * COLS
    ...
    const WEIGHTS = [800] as const
    const STYLES = ['italic'] as const
    ...
    for (const style of STYLES) {
      for (const weight of WEIGHTS) {
        const font = `italic ${weight} ${FONT_SIZE}px ${PROP_FAMILY}`
    ...
    for (let brightnessByte = 0; brightnessByte < 256; brightnessByte++) {
      ...
      brightnessLookup.push({
        propHtml: `<span class="w8 it a${alphaIndex}">${esc(match.char)}</span>`,
      })
    }
    ...
    const propBox = propBoxRef.current!
    const propRows: HTMLDivElement[] = []
    ...
    function render(now: number): void {
      ...
      for (let row = 0; row < ROWS; row++) {
        ...
        propRows[row]!.innerHTML = propHtml
      }
  ...
  return (
    <div className={twMerge('art-box', className)} ref={propBoxRef} />
  )
}
